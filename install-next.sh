...
while ! yarn global add @swipechain/core@next ; do
    read -p "Installing Swipechain Core failed, do you want to retry? [y/N]: " choice
...
swipechain config:publish --network=devnet

success "Installed Swipechain Core!"
...
if [[ "$choice" =~ ^(yes|y|Y) ]]; then
    ...
    swipechain env:set --key=CORE_DB_USERNAME --value="${databaseUsername}"
    swipechain env:set --key=CORE_DB_PASSWORD --value="${databasePassword}"
    swipechain env:set --key=CORE_DB_DATABASE --value="${databaseName}"
