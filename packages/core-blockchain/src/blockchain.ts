import { DatabaseService, Repositories } from "@swipechain/core-database";
import { Container, Contracts, Enums, Providers, Types, Utils } from "@swipechain/core-kernel";
import { DatabaseInteraction } from "@swipechain/core-state";
import { Blocks, Crypto, Interfaces, Managers } from "@swipechain/crypto";
...
export class Blockchain implements Contracts.Blockchain.Blockchain {
    ...
    @Container.tagged("plugin", "@swipechain/core-blockchain")
    ...
    public async initialize(): Promise<void> {
        ...
        if (this.stateStore.getNetworkStart()) {
            this.logger.warning(
                "Swipechain Core is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong.",
            );
        }
